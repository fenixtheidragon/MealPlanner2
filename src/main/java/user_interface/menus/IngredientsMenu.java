...
public class IngredientsMenu extends Menu implements IMenu {
	...
	private void addIngredients() {
		...
		for (String[] nameAndAmount : namesAndAmountsAL) {
			System.out.println(sqlExecutor.execute(SQLStatements.getInsertIntoStatement(TABLE_INGREDIENTS, List.of(COLUMN_INGREDIENT_NAME, COLUMN_AMOUNT_GRAMS),
	...
	private boolean checkIfIngredientExistsBy(String ingredientID) {
		return !sqlExecutor.execute(
			SQLStatements.getSelectAllColumnsFromTableWhereColumnEqualsValueStatement(TABLE_INGREDIENTS, COLUMN_INGREDIENT_ID, ingredientID)).isBlank();
	}
